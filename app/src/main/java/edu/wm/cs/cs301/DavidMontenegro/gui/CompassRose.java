...
public class CompassRose {
	...
	private void drawArms(final MazePanel panel, final int length, final int width) {
		...
		panel.setColor(MAIN_COLOR.toArgb());
        // draw each arm
	...
	private void drawBorderCircle(MazePanel panel, int width) {
		...
		// draw both arcs
		panel.setColor(CIRCLE_SHADE.toArgb());
		panel.addArc(x, y, w, w, 45, 180);
		panel.setColor(CIRCLE_HIGHLIGHT.toArgb());
	...
	private void drawDirectionMarker(MazePanel panel, int width) {
		...
		panel.setColor((CardinalDirection.South == currentDir) ? MARKER_COLOR.toArgb() : goldWM.toArgb());
		drawMarker(panel, centerX, centerY - offset, "N");

		panel.setColor((CardinalDirection.East == currentDir) ? MARKER_COLOR.toArgb() : goldWM.toArgb());
		drawMarker(panel, centerX + offset, centerY, "E");

		// WARNING: north south confusion
		// currendDir North is going downwards on the map
		panel.setColor((CardinalDirection.North == currentDir) ? MARKER_COLOR.toArgb() : goldWM.toArgb());
		drawMarker(panel, centerX, centerY + offset, "S");

		panel.setColor((CardinalDirection.West == currentDir) ? MARKER_COLOR.toArgb() : goldWM.toArgb());
